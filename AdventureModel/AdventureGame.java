...
import views.GameAction;
import views.SoundEffectManager;
...
public class AdventureGame implements Serializable {
    ...
    public boolean movePlayer(String direction) {
        ...
        for (Passage entry : possibilities) {
            ...
            if (chosen == null && entry.getIsBlocked()) {
                if (entry.getKeyName().equalsIgnoreCase("troll")) {
                    boolean result;
                    String difficulty = promptForDifficulty();
                    switch (difficulty.toLowerCase()) {
                        case "easy":
                            // Setup easy number troll if chosen easy
                            NumberTroll numTroll = new NumberTroll();
                            result = numTroll.playGame();
                            break;
                        case "medium":
                            // Setup medium cup troll if chosen medium
                            CupTroll cupTroll = new CupTroll();
                            result = cupTroll.playGame();
                            break;
                        case "hard":
                            // Setup hard cup troll if chosen hard
                            CupTrollHard cupTroll2 = new CupTrollHard();
                            result = cupTroll2.playGame();
                            break;
                        default:
                            // Invalid difficulty level forces game to assume easy level.
                            System.out.println("Invalid difficulty level. Defaulting to easy.");
                            NumberTroll defaultTroll = new NumberTroll();
                            result = defaultTroll.playGame();
                            break;
                    }
                    if (result) {
                        // Player won the troll game, grant the key
                        chosen = entry;
                        int roomNumber = chosen.getDestinationRoom();
                        Room room = this.rooms.get(roomNumber);
                        this.player.setCurrentRoom(room);
                    } else {
                        return true; // Player didn't win the troll game, can't move
                    }
                } else if (this.player.getInventory().contains(entry.getKeyName())) {
                    chosen = entry;
                    break;
                }
            } else {
            ...
            if (chosen == null && entry.getIsBlocked()) {
                if (this.player.getInventory().contains(entry.getKeyName()) || UniversalKey.keyAcquired) {
        ...
        Room room = this.rooms.get(roomNumber);
        this.player.setCurrentRoom(room);
        soundEffectManager.executeActionSound(GameAction.WALK);
    ...
    /**
     * promptForDifficulty
     * helper method for getting input for the difficulty level of trolls.
     *
     */
    private String promptForDifficulty() {
        ...
        // Polish user input to remove whitespaces
